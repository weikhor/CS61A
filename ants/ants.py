        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if (self.ant.is_container == True):
                    if (insect.is_container == False and self.ant.contained_ant == None):
                         self.ant.contain_ant(insect)
                    else:
                        assert self.ant is None, 'Two ants in {0}'.format(self)
                else:
                    if (insect.is_container == True and self.ant.is_container == False):
...
#                        assert self.ant is None, 'Two ants in {0}'.format(self)
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if self.ant.real == True:
                return 
            if (self.ant.is_container == True and self.ant.contained_ant != None and self.ant.contained_ant.real == True):
                return
#            if self.ant.real == False and self.ant.armor <= 0:
#                self.ant = None
#                return
#            if self.ant.real == False :
#                self.ant = None
#                return
#            
#            if self.ant.is_container == True and self.ant.contained_ant.real == False:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if (self.place.ant is None):
            return False
        elif (self.place.ant.name == "Ninja" ):
            return False
        else:
            return True
        # END Problem 7
...
class Ant(Insect):
    ...
    is_container = False
    contained_ant = None
    real = False #Show That this is Queen
    name = 'ant'
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if (self.is_container == True and self.contained_ant == None and other.is_container == False):
            return True
        else:
            return False          
        # END Problem 9
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        length = 0
        while (str(loc) != "Hive"):
            if (len(loc.bees) != 0 and length <= self.max_range and length >= self.min_range):
        ...
        if (self.armor <= 0):
            while (i < num_bees):
                if (self.place.bees[i].armor <= self.damage):
                    lst = lst + [self.place.bees[i]]
                else:
                    self.place.bees[i].reduce_armor(self.damage)
                i = i + 1    
                
        for bees in lst:
            self.place.bees.remove(bees)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if (self.digesting == 1 and self.counter < self.time_to_digest):
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if (QueenAnt.count == 0):
            ...
            self.real = False
        QueenAnt.count = QueenAnt.count + 1
        self.antstorage = []
        # END Problem 13
    def action(self, colony):
        ...
        if (self.real == False):
            ...
            back_insect = self.place
        
            while (back_insect != None):
                if (back_insect.ant != None and back_insect.ant != self):
                    if (back_insect.ant not in self.antstorage):
                        back_insect.ant.damage = back_insect.ant.damage*2
                        self.antstorage.append(back_insect.ant)
                        if (back_insect.ant.is_container == True and back_insect.ant.contained_ant != None and (back_insect.ant.contained_ant not in self.antstorage)):
                            back_insect.ant.contained_ant.damage = back_insect.ant.contained_ant.damage*2
                            self.antstorage.append(back_insect.ant.contained_ant)
                    
                    if (back_insect.ant.is_container == True and back_insect.ant.contained_ant != None and (back_insect.ant.contained_ant not in self.antstorage)):
                        back_insect.ant.contained_ant.damage = back_insect.ant.contained_ant.damage*2
                        self.antstorage.append(back_insect.ant.contained_ant)
                    #back_insect.ant.action(colony)
                back_insect = back_insect.exit
                #back_insect.ant.action(colony)
            super().action(colony)
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor = self.armor - amount
        if (self.armor <= 0 and self.real == True):
            bees_win()
        # END Problem 13
...
class SlowThrower(ThrowerAnt):
    ...
    # END Problem EC
    food_cost = 4  #forget press execute
    def throw_at(self, target):
        if target:
