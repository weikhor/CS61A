...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        #got see ans use schem_eval, need to use recursive and see check_procedure
        "*** YOUR CODE HERE ***"
        #print(check_procedure(env.bindings[first]))
        ...
        procedure = scheme_eval(first, env)
        check_procedure(procedure)
        return scheme_apply(procedure, expr.second.map(lambda x: scheme_eval(x,env)),  env)
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 8
    #see github expression is nil see answer to get idea (use scheme_eval(expressions.first, env)) 
    """
    ...
    if (expressions == nil):
        return None
#eval_all(expressions.second,env) == None or
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        elif self.parent is not None: # Current frame not in global frame
            return self.parent.lookup(symbol)
        raise SchemeError('unknown identifier: {0}'.format(symbol))
        
        """
        if (self.parent != None and len(self.bindings) == 0):
            return self.parent.lookup(symbol)
    ...
    def make_child_frame(self, formals, vals):
        ...
        if((formals == nil and vals != nil) or (formals != nil and vals == nil)):
            raise SchemeError("unknown identifier") 
        return new
        """
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        #have see github ans to check whether got lambda expression but do not have
        "*** YOUR CODE HERE ***"
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        #SEE GITHUB SOLUTION NEED USE env and expressions.second.first
        "*** YOUR CODE HERE ***"
        #print(expressions.second)
        #scheme_eval(expressions.second, env)
        ...
        # BEGIN PROBLEM 10
        #see github ans have to use define
        "*** YOUR CODE HERE ***"
        
        firsts  = expressions.first.first
        lambda_proc = LambdaProcedure(expressions.first.second, expressions.second, env)
        env.define(expressions.first.first, lambda_proc)
        return firsts
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    lambda_proc = LambdaProcedure(formals, expressions.second, env)
    return lambda_proc
    # END PROBLEM 9
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    #see github answer got use scheme_eval
    "*** YOUR CODE HERE ***"
    ...
    """
    elif(scheme_truep(expressions.first) == False):
        return False
    elif(expressions.second == nil):
        return scheme_eval(expressions.first, env)
    else:
        #if (expressions.first)
        #env.define(expressions.first.second.first,scheme_eval(expressions.first))
...
def do_or_form(expressions, env):
    ...
    """
    elif (scheme_symbolp(scheme_eval(expressions.first, env))):
       if (env.lookup(expressions.first) == False):
           return do_or_form(expressions.second, env)
       else:
           return env.lookup(expressions.first)
           
    
    elif(scheme_truep(expressions.first) == True):
        return scheme_eval(expressions.first, env)
    else:
        return do_or_form(expressions.second, env)
    """
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    #got see github solution no idea check_form(bindings, 2,2), bindings.second.first, env
    "*** YOUR CODE HERE ***"
    #eturn scheme_eval(bindings, env)
    new = env
    another = create_global_frame()
    
    while (bindings != nil):
        check_form(bindings.first, 2,2)
        check_formals(bindings.first.first)
        new = new.make_child_frame(Pair(bindings.first.first,nil), Pair(scheme_eval(bindings.first.second.first, env),nil))
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    "*** YOUR CODE HERE ***"
    Mu_Proc = MuProcedure(formals, expressions.second)
    return  Mu_Proc
